...
CREATE TABLE Log_Alteracoes_Salariais(
...
-- 2. Criação do Trigger de comando para registrar as mudanças salariais
CREATE OR REPLACE TRIGGER trg_log_alteracao_salario
BEFORE UPDATE ON Funcao_Salario
BEGIN
    -- Inserir um único registro informando que uma atualização foi feita
    INSERT INTO Log_Alteracoes_Salariais (Usuario, Tabela_Afetada, Tipo_Operacao)
    VALUES (USER, 'Funcao_Salario', 'UPDATE');
END;
/

-- 3. Teste da funcionalidade do TRIGGER
-- 3.1 Verificar a tabela de logs antes da alteração
SELECT * FROM Log_Alteracoes_Salariais;

-- 3.2 Atualizar múltiplos salários ao mesmo tempo
